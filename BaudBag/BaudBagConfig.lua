...
function BaudBagRestoreCfg()
	BaudBag_DebugMsg(1, "Restoring BBConfig structure:");
	
	if (type(BaudBag_Cfg) ~= "table") then
		BaudBag_DebugMsg(1, "- basic BBConfig damaged or missing, creating now");
		BaudBag_Cfg = {};
	end
	...
	for BagSet = 1, 2 do
		if (type(BBConfig[BagSet]) ~= "table") then
			BaudBag_DebugMsg(1, "- BBConfig for BagSet "..BagSet.." damaged or missing, creating now");
			BBConfig[BagSet] = {};
		end
		
    if (type(BBConfig[BagSet].Enabled) ~= "boolean") then
			BaudBag_DebugMsg(1, "- enabled state for BagSet "..BagSet.." damaged or missing, creating now");
			BBConfig[BagSet].Enabled = true;
    end
    
    if (type(BBConfig[BagSet].CloseAll) ~= "boolean") then
			BaudBag_DebugMsg(1, "- close all state for BagSet "..BagSet.." damaged or missing, creating now");
			BBConfig[BagSet].CloseAll = true;
    end
    
    if (type(BBConfig[BagSet].Joined) ~= "table") then
			BaudBag_DebugMsg(1, "- joins for BagSet "..BagSet.." damaged or missing, creating now");
			BBConfig[BagSet].Joined = {};
    end
    
    if (type(BBConfig[BagSet].ShowBags) ~= "boolean") then
			BaudBag_DebugMsg(1, "- show information for BagSet "..BagSet.." damaged or missing, creating now");
    ...
    BaudBagForEachBag(BagSet, function(Bag, Index)
      ...
      if (Bag == -2) and (BBConfig[BagSet].Joined[Index] == nil) then
				BaudBag_DebugMsg(1, "- BagSet["..BagSet.."], Bag["..Bag.."] joined status damaged or missing, creating now");
      ...
      if (Container == 0) or (BBConfig[BagSet].Joined[Index] == false) then
        ...
        if (type(BBConfig[BagSet][Container]) ~= "table") then
					BaudBag_DebugMsg(1, "- BagSet["..BagSet.."], Bag["..Bag.."], Container["..Container.."] container data damaged or missing, creating now");
        ...
        if not BBConfig[BagSet][Container].Name then
					BaudBag_DebugMsg(1, "- BagSet["..BagSet.."], Bag["..Bag.."], Container["..Container.."] container name missing, creating now");
        ...
        for Key, Value in ipairs(SliderBars)do
          if (type(BBConfig[BagSet][Container][Value.SavedVar]) ~= "number") then
			BaudBag_DebugMsg(1, "- BagSet["..BagSet.."], Bag["..Bag.."], Container["..Container.."] Slider["..Value.SavedVar.."] data damaged or missing, creating now");
        ...
        for Key, Value in ipairs(CheckButtons)do
          if (type(BBConfig[BagSet][Container][Value.SavedVar]) ~= "boolean") then
			BaudBag_DebugMsg(1, "- BagSet["..BagSet.."], Bag["..Bag.."], Container["..Container.."] CheckBox["..Value.SavedVar.."] data damaged or missing, creating now");

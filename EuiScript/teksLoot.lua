...
local backdrop = {
	bgFile = E.blank, tile = true, tileSize = 0,
	edgeFile = E.blank, edgeSize = E.mult,
...
local function CreateRollFrame()
	...
	status:SetStatusBarTexture(E.normTex)
...
local rollpairs = locale == "zhTW" and {
	["(.*)自動放棄:(.+)，因為"]  = "pass",
	["(.*)放棄了:(.+)"] = "pass",
	["(.*)選擇了貪婪優先:(.+)"] = "greed",
	["(.*)選擇了需求優先:(.+)"] = "need",
...
anchor:SetScript("OnEvent", function(frame, event, addon)
	if addon ~= "EuiScript" then return end

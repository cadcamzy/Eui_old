...
local function CreateVirtualFrame(parent, point)
	...
	parent.backdrop2:SetTexture(.07,.07,.07)	
	...
	parent.bordertop:SetTexture(.23,.23,.23)	
	...
	parent.borderbottom:SetTexture(.23,.23,.23)	
	...
	parent.borderleft:SetTexture(.23,.23,.23)	
	...
	parent.borderright:SetTexture(.23,.23,.23)	
...
local function UpdateObjects(frame)
	...
	SetVirtualBorder(frame.hp, .23,.23,.23)
...
local goodR, goodG, goodB = C["nameplate"].goodcolor.r, C["nameplate"].goodcolor.g, C["nameplate"].goodcolor.b
local badR, badG, badB = C["nameplate"].badcolor.r, C["nameplate"].badcolor.g, C["nameplate"].badcolor.b
local transitionR, transitionG, transitionB = C["nameplate"].goodtransitioncolor.r, C["nameplate"].goodtransitioncolor.g, C["nameplate"].goodtransitioncolor.b
local transitionR2, transitionG2, transitionB2 = C["nameplate"].badtransitioncolor.r, C["nameplate"].badtransitioncolor.g, C["nameplate"].badtransitioncolor.b
		...
		if not frame.region:IsShown() then
			if InCombatLockdown() and frame.isFriendly ~= true then
				...
				if E.Role == "Tank" then
					...
					frame.hp.hpbg:SetTexture(badR, badG, badB, 0.15)
					frame.threatStatus = "BAD"
				else
					frame.hp:SetStatusBarColor(goodR, goodG, goodB)
					frame.hp.hpbg:SetTexture(goodR, goodG, goodB, 0.15)
					frame.threatStatus = "GOOD"
				end		
			else
				--Set colors to their original, not in combat
				frame.hp:SetStatusBarColor(frame.hp.rcolor, frame.hp.gcolor, frame.hp.bcolor)
				frame.hp.hpbg:SetTexture(frame.hp.rcolor, frame.hp.gcolor, frame.hp.bcolor, 0.15)
				frame.threatStatus = nil
			end
		else
			...
			if g + b == 0 then
				...
				if E.Role == "Tank" then
					if frame.threatStatus == "GOOD" then
						...
						frame.hp.hpbg:SetTexture(transitionR2, transitionG2, transitionB2, 0.15)				
					else
						--Gaining Threat
						frame.hp:SetStatusBarColor(transitionR, transitionG, transitionB)	
						frame.hp.hpbg:SetTexture(transitionR, transitionG, transitionB, 0.15)	
					end
				else
					if frame.threatStatus == "GOOD" then
						--Losing Threat

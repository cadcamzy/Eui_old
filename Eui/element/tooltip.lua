...
--	local inInstance, instanceType = IsInInstance()
 	if self:GetAnchorType() == "ANCHOR_CURSOR" then
		...
		self:ClearAllPoints();
		self:SetPoint("BOTTOMLEFT","UIParent","BOTTOMLEFT",(x / effScale + (C["tooltip"].x)),(y / effScale + (C["tooltip"].y)))		
		if InCombatLockdown() and C["tooltip"].hideincombat == true then self:Hide() end
	end
...
--[[ 	if (C["tooltip"].ShowTalent and UnitIsPlayer(unit)) then
		...
		inspectList[name] = inspectList[name] or {} --去除缓存
	--	inspectList[name] = {}
		if #inspectList > 30 then inspectList = {} end
...
--EuiTooltip:RegisterEvent("INSPECT_READY")

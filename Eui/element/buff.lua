...
local holder = CreateFrame("Frame", "EuiAurasHolder", UIParent)
holder:SetPoint("TOPRIGHT", Minimap, "TOPLEFT", E.Scale(-8), E.Scale(2))
holder:SetWidth(E.Scale(456)) --(30 + 8) * 12)
holder:SetHeight(Minimap:GetHeight() + E.Scale(3 + 19))

local btnspace = E.Scale(-4)
local aurapos = "RIGHT"

TemporaryEnchantFrame:ClearAllPoints()
TemporaryEnchantFrame:SetPoint("TOPRIGHT", EuiAurasHolder, "TOPRIGHT", 0, 0)

TempEnchant1:ClearAllPoints()
TempEnchant2:ClearAllPoints()
TempEnchant1:SetPoint("TOPRIGHT", EuiAurasHolder, "TOPRIGHT", 0, 0)
TempEnchant2:SetPoint("RIGHT", TempEnchant1, "LEFT", E.Scale(-4), 0)

for i = 1, 3 do
	local f = CreateFrame("Frame", nil, _G["TempEnchant"..i])
	E.EuiCreatePanel(f, 30, 30, "CENTER", _G["TempEnchant"..i], "CENTER", 0, 0)	
			...
			if aurapos == "RIGHT" then
				if ( (index > 1) and (mod(index, rowbuffs) == 1) ) then
					if ( index == rowbuffs+1 ) then
						buff:SetPoint("RIGHT", EuiAurasHolder, "RIGHT", 0, 0)
					else
						buff:SetPoint("TOPRIGHT", EuiAurasHolder, "TOPRIGHT", 0, 0)
					end
					aboveBuff = buff;
				elseif ( index == 1 ) then
				...
				if ( (index > 1) and (mod(index, rowbuffs) == 1) ) then
					if ( index == rowbuffs+1 ) then
						buff:SetPoint("LEFT", EuiAurasHolder, "LEFT", 0, 0)
					else
						buff:SetPoint("TOPLEFT", EuiAurasHolder, "TOPLEFT", 0, 0)
					end
					aboveBuff = buff;
				elseif ( index == 1 ) then
					local mainhand, _, _, offhand, _, _, hand3 = GetWeaponEnchantInfo()
...
local function UpdateDebuffAnchors(buttonName, index)
	...
	if aurapos == "RIGHT" then
		if index == 1 then
			debuff:SetPoint("BOTTOMRIGHT", EuiAurasHolder, "BOTTOMRIGHT", 0, 0)
		else
			debuff:SetPoint("RIGHT", _G[buttonName..(index-1)], "LEFT", btnspace, 0)
		end
	else
		if index == 1 then
			debuff:SetPoint("BOTTOMLEFT", EuiAurasHolder, "BOTTOMLEFT", 0, 0)
		else
...
function E.AurasPostDrag(frame)
	...
	if string.find(point, "LEFT") then
		...
		btnspace = E.Scale(4)
		aurapos = "LEFT"
		TempEnchant1:SetPoint("TOPLEFT", EuiAurasHolder, "TOPLEFT", 0, 0)
		TempEnchant2:SetPoint("LEFT", TempEnchant1, "RIGHT", btnspace, 0)		
		TemporaryEnchantFrame:SetPoint("TOPLEFT", EuiAurasHolder, "TOPLEFT", 0, 0)	
...
E.CreateMover(EuiAurasHolder, "AurasMover", L.BUFF, false, E.AurasPostDrag)

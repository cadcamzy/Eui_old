...
local function StyleTotemOpenButton(button, parent)
	...
	if not button.visibleBut then
		...
		button.visibleBut:SetWidth(C["actionbar"].petbuttonsize)
...
local function StyleTotemSlotButton(button, index)
	...
	if not InCombatLockdown() then button:SetSize(C["actionbar"].petbuttonsize,C["actionbar"].petbuttonsize) end

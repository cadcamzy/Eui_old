...
local specWarnInfest		= mod:NewSpecialWarning("specWarnInfest", mod:IsHealer()) --Phase 1+ Ability
local specWarnShamblingFrenzy	= mod:NewSpecialWarningSpell(28747, mod:IsHealer() or mod:IsTank())
local specwarnSoulreaper	= mod:NewSpecialWarning("specWarnSoulreaper", mod:IsTank() or mod:IsHealer()) --phase 2+
...
local timerSoulreaperCD	 	= mod:NewTimer(30.5, "TimerSoulreaperCD", 73797, mod:IsTank() or mod:IsHealer())
local timerHarvestSoul	 	= mod:NewTargetTimer(6, 74325)
local timerHarvestSoulCD	= mod:NewNextTimer(75, 74325)
local timerInfestCD			= mod:NewTimer(22.5, "TimerInfestCD", 73779, mod:IsHealer())
...
local timerEnrageCD			= mod:NewCDTimer(20, 72143, nil, mod:IsTank() or mod:CanRemoveEnrage() or mod:IsHealer())
...
function mod:SPELL_CAST_SUCCESS(args)
	if args:IsSpellID(70337, 73912, 73913, 73914) then -- Necrotic Plague (SPELL_AURA_APPLIED is not fired for this spell)
		...
		elseif canPurge then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\plague.mp3")
		end
	...
	function mod:SPELL_AURA_APPLIED(args)
		...
		elseif args:IsSpellID(28747) then -- Shambling Horror Frenzy
			...
			if mod:IsHealer() or mod:IsTank() then
				sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\frenzy.mp3")
			end

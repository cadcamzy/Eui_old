...
function mod:ApplySettings(win)
	...
	g.button:SetBackdrop({
	  bgFile = E.normTex, 
	  edgeFile = E.normTex, 
	  tile = false, tileSize = 0, edgeSize = 1, 
	  insets = { left = -1, right = -1, top = -1, bottom = -1}
	})
	g.button:SetBackdropColor(0.1,0.1,0.1,0.9)
	g.button:SetBackdropBorderColor(0.31, 0.45, 0.63,.3)
	...
	if p.enablebackground then
		if g.bgframe == nil then
			...
			FrameBackdrop = CreateFrame("Frame", nil, g.bgframe)
		...
		windowbackdrop.bgFile = "Interface\\AddOns\\eMedia\\Blank"
		windowbackdrop.edgeFile = ""
		windowbackdrop.tile = false
		windowbackdrop.tileSize = 0
		windowbackdrop.edgeSize = 0
		windowbackdrop.insets = {0, 0, 0, 0}
		...
		if p.reversegrowth then
			g.bgframe:SetPoint("LEFT", g.button, "LEFT", -2, 0)
			g.bgframe:SetPoint("RIGHT", g.button, "RIGHT", 2, 0)
			g.bgframe:SetPoint("TOP", g.button, "TOP", 0, 2)
		else
			g.bgframe:SetPoint("LEFT", g.button, "LEFT", -2, 0)
			g.bgframe:SetPoint("RIGHT", g.button, "RIGHT", 2, 0)
			g.bgframe:SetPoint("TOP", g.button, "TOP", 0, 2)
		...
		FrameBackdrop:SetPoint("LEFT", g.bgframe, "LEFT", -p.background.borderthickness+3, 0)
		FrameBackdrop:SetPoint("RIGHT", g.bgframe, "RIGHT", p.background.borderthickness-3, 0)
		FrameBackdrop:SetPoint("TOP", g.bgframe, "TOP", 0, 5)
		FrameBackdrop:SetPoint("BOTTOM", g.bgframe, "BOTTOM", 0, -5)
		FrameBackdrop:SetFrameStrata("BACKGROUND")
		...
		if p.background.height > 0 then
			local maxbars = math.floor(p.background.height / math.max(1, p.barheight + p.barspacing) -1)
